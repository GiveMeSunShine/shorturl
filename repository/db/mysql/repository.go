...
func CreateTable(engine *xorm.Engine) {
	...
	tableSql := "CREATE TABLE if not exists `url_map`( " +
		...
		"`type` VARCHAR(8) NOT NULL COMMENT '类型 0 长链接 1 文件', " +
...
func (m *mysqlRepository) Find(code string) (dbStore *service.DBStore, err error) {
	log.Println("[MySQL] Find : ", code)
	findSql := fmt.Sprintf("select short_url,long_url,compression_code,type,file_name from url_map where compression_code='%s' and  deleted=0 and url_status=1",
		code)
	row := m.client.DB().QueryRow(findSql)
	dbStore = new(service.DBStore)
	err = row.Scan(&dbStore.ShortUrl, &dbStore.OrgLink, &dbStore.Code, &dbStore.Type, &dbStore.FileName)
	if err != nil {
		return nil, err
	}
	return dbStore, nil
...
func (m *mysqlRepository) Store(dbStore *service.DBStore) ([]byte, error) {
	insertSql := fmt.Sprintf("insert into url_map (short_url,long_url,compression_code,type,file_name) values('%s','%s','%s','%s','%s') ",
		dbStore.ShortUrl, dbStore.OrgLink, dbStore.Code, dbStore.Type, dbStore.FileName)
