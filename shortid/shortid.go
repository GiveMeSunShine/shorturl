...
// Package shortid enables the generation of shortid, unique, non-sequential and by default URL friendly
...
// Shortid type represents a shortid Id generator working with a given alphabet.
type Shortid struct {
	abc    Abc
	worker uint
...
// GetDefault retrieves the default shortid Id generator initialised with the default alphabet,
...
// ShortIdFromCode acts just like Generate, but panics instead of returning errors.
func ShortIdFromCode() string {
	id, err := Generate()
	if err == nil {
		return id
	}
...
// New constructs an instance of the shortid Id generator for the given worker number [0,31], alphabet
...
// Generate generates a new shortid Id.
...
// String returns a string representation of the shortid Id generator.
...
// Worker returns the value of worker for this shortid Id generator.
